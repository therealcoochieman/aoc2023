...
const EXPANSION: u64 = 1;

fn manhattan_galaxy(galaxies: Vec<Coordinates>) {
    let mut thing = 0;
...
impl Expansion {
    pub fn compute_index(&self, index: usize) -> u64 {
        ...
        let closest_expansion = closest_expansion.unwrap();

        index as u64 + closest_expansion.1
    }
    ...
    pub fn add_expansion(&mut self, index: usize, expansion_value: u64) {
        ...
        for i in position + 1..self.expansion.len() {
            self.expansion[i].1 += expansion_value;
        }
...
fn expand_space(lines: &mut Vec<Vec<char>>) -> Vec<Coordinates> {
    ...
    x_expansion.add_expansion(0, 0);
    y_expansion.add_expansion(0, 0);
    ...
    while x < x_len {
        ...
        if galaxy_count == 0 {
            x_expansion.add_expansion(x, EXPANSION);
        }
    ...
    while y < y_len {
        ...
        if thing == 0 {
            y_expansion.add_expansion(y, EXPANSION);
        }
    ...
    for (y, line) in lines.iter().enumerate() {
        for (x, &c) in line.iter().enumerate() {
            if c == '#' {
                ...
                println!("galaxy in {}, {}", galaxy.0 +1, galaxy.1 + 1);
